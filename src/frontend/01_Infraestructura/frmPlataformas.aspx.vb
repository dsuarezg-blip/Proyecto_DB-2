...
Partial Public Class frmPlataformas
    ...
    Private Sub CargarGrid(idAeropuerto As Object)
        Try
            ...
            If idAeropuerto IsNot Nothing AndAlso idAeropuerto.ToString() <> "" Then
                ...
                dt = DatabaseHelper.ExecuteCursorReader("PKG_AERO_PLATAFORMAS", "pr_listar_por_aeropuerto", params)
            Else
                dt = DatabaseHelper.ExecuteCursorReader("PKG_AERO_PLATAFORMAS", "pr_listar", New List(Of OracleParameter))
            End If
            gvPlataformas.DataSource = dt
            gvPlataformas.DataBind()
            lblConteo.Text = String.Format("{0} registro(s)", dt.Rows.Count)
        Catch ex As Exception
            MostrarToast("danger", "Error al cargar plataformas: " & ex.Message)
        End Try
    End Sub
    ...
    Protected Sub gvPlataformas_RowCommand(sender As Object, e As GridViewCommandEventArgs)
    ...
    Private Sub CargarParaEditar(id As Integer)
        Try
            Dim dt = DatabaseHelper.ExecuteCursorReader("PKG_AERO_PLATAFORMAS", "pr_listar", New List(Of OracleParameter))
            Dim rows = dt.Select(String.Format("ID_PLATAFORMA = {0}", id))
            If rows.Length > 0 Then
                Dim row = rows(0)
                hfIdPlataforma.Value = id.ToString()
    ...
    Protected Sub btnGuardar_Click(sender As Object, e As EventArgs)
        Try
            Dim id As Integer = Convert.ToInt32(hfIdPlataforma.Value)
            Dim params As New List(Of OracleParameter)
            If id = 0 Then
                ...
                DatabaseHelper.ExecuteNonQuery("PKG_AERO_PLATAFORMAS", "pr_insertar", params)
                MostrarToast("success", "Plataforma registrada correctamente.")
            Else
                params.Add(New OracleParameter("p_id_plataforma", OracleDbType.Int32) With {.Value = id})
                params.Add(New OracleParameter("p_id_aeropuerto", OracleDbType.Int32) With {.Value = Convert.ToInt32(If(ddlAeropuertoModal.SelectedValue <> "", ddlAeropuertoModal.SelectedValue, If(Session("ModalAeropuertoId") IsNot Nothing, Session("ModalAeropuertoId").ToString(), "0")))})
                params.Add(New OracleParameter("p_nombre", OracleDbType.Varchar2) With {.Value = txtNombre.Text.Trim()})
                params.Add(New OracleParameter("p_tipo", OracleDbType.Varchar2) With {.Value = ddlTipo.SelectedValue})
                params.Add(New OracleParameter("p_capacidad_aeros", OracleDbType.Int32) With {.Value = If(txtCapacidad.Text.Trim() = "", DBNull.Value, Convert.ToInt32(txtCapacidad.Text.Trim()))})
                ...
                DatabaseHelper.ExecuteNonQuery("PKG_AERO_PLATAFORMAS", "pr_actualizar", params)
                MostrarToast("success", "Plataforma actualizada correctamente.")
            End If
            hfIdPlataforma.Value = "0"
    ...
    Private Sub Desactivar(id As Integer)
        Try
            ...
            If rows.Length > 0 Then
                ...
                DatabaseHelper.ExecuteNonQuery("PKG_AERO_PLATAFORMAS", "pr_actualizar", params)
                MostrarToast("success", "Plataforma desactivada correctamente.")
                CargarGrid(Nothing)
            End If
        Catch ex As Exception
    ...
    Private Sub LimpiarFormulario()
        ...
        ddlTipo.SelectedIndex = 0 : ddlActivo.SelectedValue = "S" : hfIdPlataforma.Value = "0"
    End Sub
