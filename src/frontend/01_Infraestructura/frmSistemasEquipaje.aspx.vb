...
Partial Public Class frmSistemasEquipaje
    ...
    Private Sub CargarGrid(idTerminal As Object)
        Try
            Dim dt = DatabaseHelper.ExecuteCursorReader("PKG_AERO_SISTEMAS_EQUIPAJE", "pr_listar", New List(Of OracleParameter))
            ...
            gvEquipaje.DataBind()
        Catch ex As Exception
            MostrarToast("danger", "Error al cargar sistemas: " & ex.Message)
        End Try
    End Sub
    ...
    Protected Sub gvEquipaje_RowCommand(sender As Object, e As GridViewCommandEventArgs)
    ...
    Private Sub CargarParaEditar(id As Integer)
        Try
            Dim dt = DatabaseHelper.ExecuteCursorReader("PKG_AERO_SISTEMAS_EQUIPAJE", "pr_listar", New List(Of OracleParameter))
            Dim rows = dt.Select(String.Format("ID_BANDA = {0}", id))
            If rows.Length > 0 Then
                Dim row = rows(0)
                hfIdBanda.Value = id.ToString()
                txtCodigo.Text = row("CODIGO").ToString()
                txtLongitud.Text = row("LONGITUD_M").ToString()
                txtCapacidad.Text = row("CAPACIDAD_MALETAS_HORA").ToString()
    ...
    Protected Sub btnGuardar_Click(sender As Object, e As EventArgs)
        Try
            Dim id As Integer = Convert.ToInt32(hfIdBanda.Value)
            ...
            If id = 0 Then
                ...
                params.Add(New OracleParameter("p_capacidad_mal", OracleDbType.Int32) With {.Value = If(txtCapacidad.Text.Trim() = "", DBNull.Value, Convert.ToInt32(txtCapacidad.Text.Trim()))})
                DatabaseHelper.ExecuteNonQuery("PKG_AERO_SISTEMAS_EQUIPAJE", "pr_insertar", params)
                MostrarToast("success", "Sistema registrado correctamente.")
            Else
                params.Add(New OracleParameter("p_id_banda", OracleDbType.Int32) With {.Value = id})
                params.Add(New OracleParameter("p_id_terminal", OracleDbType.Int32) With {.Value = Convert.ToInt32(If(ddlTerminalModal.SelectedValue <> "", ddlTerminalModal.SelectedValue, If(Session("ModalTerminalId") IsNot Nothing, Session("ModalTerminalId").ToString(), "0")))})
                params.Add(New OracleParameter("p_codigo", OracleDbType.Varchar2) With {.Value = txtCodigo.Text.Trim()})
                params.Add(New OracleParameter("p_tipo", OracleDbType.Varchar2) With {.Value = ddlTipo.SelectedValue})
                params.Add(New OracleParameter("p_longitud_m", OracleDbType.Decimal) With {.Value = If(txtLongitud.Text.Trim() = "", DBNull.Value, Convert.ToDecimal(txtLongitud.Text.Trim()))})
                params.Add(New OracleParameter("p_capacidad_mal", OracleDbType.Int32) With {.Value = If(txtCapacidad.Text.Trim() = "", DBNull.Value, Convert.ToInt32(txtCapacidad.Text.Trim()))})
                params.Add(New OracleParameter("p_estado", OracleDbType.Varchar2) With {.Value = "OPERATIVO"})
                params.Add(New OracleParameter("p_activo", OracleDbType.Char) With {.Value = ddlActivo.SelectedValue})
                DatabaseHelper.ExecuteNonQuery("PKG_AERO_SISTEMAS_EQUIPAJE", "pr_actualizar", params)
                MostrarToast("success", "Sistema actualizado correctamente.")
            End If
            hfIdBanda.Value = "0" : divActivo.Visible = False : LimpiarFormulario() : CargarGrid(Nothing)
        Catch ex As Exception
    ...
    Private Sub Desactivar(id As Integer)
        Try
            ...
            If rows.Length > 0 Then
                ...
                params.Add(New OracleParameter("p_capacidad_mal", OracleDbType.Int32) With {.Value = If(row.IsNull("CAPACIDAD_MALETAS_HORA"), DBNull.Value, row("CAPACIDAD_MALETAS_HORA"))})
                params.Add(New OracleParameter("p_estado", OracleDbType.Varchar2) With {.Value = If(row.IsNull("ESTADO"), "OPERATIVO", row("ESTADO").ToString())})
                params.Add(New OracleParameter("p_activo", OracleDbType.Char) With {.Value = "N"})
                DatabaseHelper.ExecuteNonQuery("PKG_AERO_SISTEMAS_EQUIPAJE", "pr_actualizar", params)
                MostrarToast("success", "Sistema desactivado correctamente.")
                CargarGrid(Nothing)
            End If
        Catch ex As Exception
    ...
    Private Sub LimpiarFormulario()
        txtCodigo.Text = "" : txtLongitud.Text = "" : txtCapacidad.Text = ""
        ddlTipo.SelectedIndex = 0 : ddlActivo.SelectedValue = "S" : hfIdBanda.Value = "0"
    End Sub
