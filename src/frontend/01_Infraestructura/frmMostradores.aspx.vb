...
Partial Public Class frmMostradores
    ...
    Private Sub CargarGrid(idTerminal As Object)
        Try
            Dim dt = DatabaseHelper.ExecuteCursorReader("PKG_AERO_MOSTRADORES", "pr_listar", New List(Of OracleParameter))
            If idTerminal IsNot Nothing AndAlso idTerminal.ToString() <> "" Then
                ...
                gvMostradores.DataSource = dtF
                lblConteo.Text = String.Format("{0} registro(s)", dtF.Rows.Count)
            Else
                gvMostradores.DataSource = dt
                lblConteo.Text = String.Format("{0} registro(s)", dt.Rows.Count)
            End If
            gvMostradores.DataBind()
        Catch ex As Exception
            MostrarToast("danger", "Error al cargar mostradores: " & ex.Message)
        End Try
    End Sub
    ...
    Protected Sub gvMostradores_RowCommand(sender As Object, e As GridViewCommandEventArgs)
    ...
    Private Sub CargarParaEditar(id As Integer)
        Try
            Dim dt = DatabaseHelper.ExecuteCursorReader("PKG_AERO_MOSTRADORES", "pr_listar", New List(Of OracleParameter))
            Dim rows = dt.Select(String.Format("ID_COUNTER = {0}", id))
            If rows.Length > 0 Then
                Dim row = rows(0)
                hfIdCounter.Value = id.ToString()
    ...
    Protected Sub btnGuardar_Click(sender As Object, e As EventArgs)
        Try
            ...
            Else
                ...
                DatabaseHelper.ExecuteNonQuery("PKG_AERO_MOSTRADORES", "pr_actualizar", params)
                MostrarToast("success", "Mostrador actualizado correctamente.")
            End If
            hfIdCounter.Value = "0" : divActivo.Visible = False : LimpiarFormulario() : CargarGrid(Nothing)
        Catch ex As Exception
    ...
    Private Sub Desactivar(id As Integer)
        Try
            ...
            If rows.Length > 0 Then
                ...
                DatabaseHelper.ExecuteNonQuery("PKG_AERO_MOSTRADORES", "pr_actualizar", params)
                MostrarToast("success", "Mostrador desactivado correctamente.")
                CargarGrid(Nothing)
            End If
        Catch ex As Exception
    ...
    Private Sub LimpiarFormulario()
        txtCodigo.Text = "" : ddlTipo.SelectedIndex = 0 : ddlActivo.SelectedValue = "S" : hfIdCounter.Value = "0"
    End Sub
