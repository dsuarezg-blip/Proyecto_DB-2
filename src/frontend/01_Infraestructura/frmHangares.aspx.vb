...
Partial Public Class frmHangares
    ...
    Private Sub CargarGrid(idAeropuerto As Object)
        Try
            ...
            If idAeropuerto IsNot Nothing AndAlso idAeropuerto.ToString() <> "" Then
                ...
                dt = DatabaseHelper.ExecuteCursorReader("PKG_AERO_HANGARES", "pr_listar_disponibles", params)
            Else
                dt = DatabaseHelper.ExecuteCursorReader("PKG_AERO_HANGARES", "pr_listar", New List(Of OracleParameter))
            End If
            gvHangares.DataSource = dt
            gvHangares.DataBind()
            lblConteo.Text = String.Format("{0} registro(s)", dt.Rows.Count)
        Catch ex As Exception
            MostrarToast("danger", "Error al cargar hangares: " & ex.Message)
        End Try
    End Sub
    ...
    Protected Sub gvHangares_RowCommand(sender As Object, e As GridViewCommandEventArgs)
    ...
    Private Sub CargarParaEditar(id As Integer)
        Try
            Dim dt = DatabaseHelper.ExecuteCursorReader("PKG_AERO_HANGARES", "pr_listar", New List(Of OracleParameter))
            Dim rows = dt.Select(String.Format("ID_HANGAR = {0}", id))
            If rows.Length > 0 Then
                Dim row = rows(0)
                hfIdHangar.Value = id.ToString()
                txtNombre.Text = row("NOMBRE").ToString()
                txtCapacidad.Text = row("CAPACIDAD_AERONAVES").ToString()
                txtArea.Text = row("AREA_M2").ToString()
                txtAltura.Text = row("ALTURA_M").ToString()
    ...
    Protected Sub btnGuardar_Click(sender As Object, e As EventArgs)
        Try
            ...
            If id = 0 Then
                ...
                params.Add(New OracleParameter("p_altura_m", OracleDbType.Decimal) With {.Value = If(txtAltura.Text.Trim() = "", DBNull.Value, Convert.ToDecimal(txtAltura.Text.Trim()))})
                DatabaseHelper.ExecuteNonQuery("PKG_AERO_HANGARES", "pr_insertar", params)
            ...
            Else
                ...
                params.Add(New OracleParameter("p_area_m2", OracleDbType.Decimal) With {.Value = If(txtArea.Text.Trim() = "", DBNull.Value, Convert.ToDecimal(txtArea.Text.Trim()))})
                params.Add(New OracleParameter("p_altura_m", OracleDbType.Decimal) With {.Value = If(txtAltura.Text.Trim() = "", DBNull.Value, Convert.ToDecimal(txtAltura.Text.Trim()))})
                params.Add(New OracleParameter("p_disponible", OracleDbType.Char) With {.Value = "S"})
                params.Add(New OracleParameter("p_activo", OracleDbType.Char) With {.Value = ddlActivo.SelectedValue})
                DatabaseHelper.ExecuteNonQuery("PKG_AERO_HANGARES", "pr_actualizar", params)
                MostrarToast("success", "Hangar actualizado correctamente.")
            End If
            hfIdHangar.Value = "0"
    ...
    Private Sub Desactivar(id As Integer)
        Try
            ...
            If rows.Length > 0 Then
                ...
                params.Add(New OracleParameter("p_capacidad_aeros", OracleDbType.Int32) With {.Value = If(row.IsNull("CAPACIDAD_AERONAVES"), DBNull.Value, row("CAPACIDAD_AERONAVES"))})
                params.Add(New OracleParameter("p_area_m2", OracleDbType.Decimal) With {.Value = If(row.IsNull("AREA_M2"), DBNull.Value, row("AREA_M2"))})
                params.Add(New OracleParameter("p_altura_m", OracleDbType.Decimal) With {.Value = If(row.IsNull("ALTURA_M"), DBNull.Value, row("ALTURA_M"))})
                params.Add(New OracleParameter("p_disponible", OracleDbType.Char) With {.Value = row("DISPONIBLE").ToString()})
                params.Add(New OracleParameter("p_activo", OracleDbType.Char) With {.Value = "N"})
                DatabaseHelper.ExecuteNonQuery("PKG_AERO_HANGARES", "pr_actualizar", params)
                MostrarToast("success", "Hangar desactivado correctamente.")
                CargarGrid(Nothing)
            End If
        Catch ex As Exception
    ...
    Private Sub LimpiarFormulario()
        txtNombre.Text = "" : txtCapacidad.Text = "" : txtArea.Text = "" : txtAltura.Text = ""
        ddlTipo.SelectedIndex = 0 : ddlActivo.SelectedValue = "S" : hfIdHangar.Value = "0"
    End Sub
