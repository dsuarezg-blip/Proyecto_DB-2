<%@ Page Title="Mostradores" Language="VB" MasterPageFile="~/Site.Master" AutoEventWireup="false" CodeFile="frmMostradores.aspx.vb" Inherits="frmMostradores" %>
<asp:Content ID="Content1" ContentPlaceHolderID="TitleContent" runat="server">Mostradores</asp:Content>
...
<style>
    ...
    .modal-box { background:#fff; border-radius:12px; padding:2rem; max-width:520px; margin:5% auto; }
...
<div class="container-fluid px-4 py-3">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h2 class="fw-bold mb-0" style="color:#0d2040;"><i class="bi bi-shop-fill me-2" style="color:#c5a059;"></i>Mostradores</h2>
            <nav aria-label="breadcrumb"><ol class="breadcrumb mb-0 small">
                <li class="breadcrumb-item"><a href="/06_Seguridad_Finanzas/Dashboard.aspx">Inicio</a></li>
                <li class="breadcrumb-item active">Mostradores</li>
            </ol></nav>
        </div>
        <asp:Panel ID="pnlBtnNuevo" runat="server">
            <button type="button" class="btn btn-sm text-white" style="background:#c5a059;" onclick="abrirModal()">
                <i class="bi bi-plus-lg me-1"></i> Nuevo Mostrador
            </button>
        </asp:Panel>
    </div>
    ...
    <div class="card border-0 shadow-sm">
        <div class="card-header bg-white d-flex justify-content-between align-items-center py-3">
            <h6 class="fw-bold mb-0 text-secondary"><i class="bi bi-list-ul me-2"></i>Listado de Mostradores</h6>
            <asp:Label ID="lblConteo" runat="server" CssClass="badge bg-light text-dark border"></asp:Label>
        </div>
        <div class="card-body p-0">
            <div class="table-responsive">
                <asp:GridView ID="gvMostradores" runat="server" CssClass="table table-hover align-middle mb-0"
                    AutoGenerateColumns="false" EmptyDataText="No hay mostradores para mostrar."
                    DataKeyNames="ID_COUNTER" OnRowCommand="gvMostradores_RowCommand">
                    ...
                    <Columns>
                        ...
                        <asp:TemplateField HeaderText="Acciones">
                            <ItemTemplate>
                                <asp:LinkButton runat="server" CommandName="Editar" CommandArgument='<%# Eval("ID_COUNTER") %>' CssClass="btn btn-xs btn-outline-primary me-1" Text="✏️"></asp:LinkButton>
                                <asp:Panel ID="pnlDes" runat="server" style="display:inline-block;" Visible='<%# Session("UserRole") IsNot Nothing AndAlso Session("UserRole").ToString().ToUpper() = "ADMIN" AndAlso Eval("ACTIVO").ToString() = "S" %>'>
                                    <asp:LinkButton runat="server" CommandName="Eliminar" CommandArgument='<%# Eval("ID_COUNTER") %>' CssClass="btn btn-xs btn-outline-danger" Text="⊘" OnClientClick="return confirm('¿Desactivar este mostrador?');"></asp:LinkButton>
                                </asp:Panel>
                            </ItemTemplate>
                        </asp:TemplateField>
                    </Columns>
...
<div id="modalMostrador" class="modal-overlay">
    <div class="modal-box">
        <div class="d-flex justify-content-between align-items-center mb-4">
            <h5 class="fw-bold mb-0" style="color:#0d2040;" id="modalTitulo">Nuevo Mostrador</h5>
            <button type="button" class="btn-close" onclick="cerrarModal()"></button>
        </div>
        <asp:HiddenField ID="hfIdCounter" runat="server" Value="0" />
        <div class="row g-3">
            <div class="col-md-12">
            ...
            <div class="col-md-6">
                ...
                <asp:TextBox ID="txtCodigo" runat="server" CssClass="form-control form-control-sm" placeholder="Ej: M01"></asp:TextBox>
            </div>
            <div class="col-md-6">
                <label class="form-label small fw-semibold">Tipo</label>
                <asp:DropDownList ID="ddlTipo" runat="server" CssClass="form-select form-select-sm">
                    <asp:ListItem Text="Check-in" Value="CHECK_IN"></asp:ListItem>
                    <asp:ListItem Text="Migración" Value="MIGRACION"></asp:ListItem>
                    <asp:ListItem Text="Información" Value="INFORMACION"></asp:ListItem>
...
<script>
    function abrirModal() { document.getElementById('modalMostrador').classList.add('active'); document.getElementById('modalTitulo').innerText = 'Nuevo Mostrador'; }
    function cerrarModal() { document.getElementById('modalMostrador').classList.remove('active'); }
    function abrirModalEditar() { document.getElementById('modalTitulo').innerText = 'Editar Mostrador'; document.getElementById('modalMostrador').classList.add('active'); }
</script>
</asp:Content>
